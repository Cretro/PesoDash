...
const CAT_COLORS = { Food: "#95C159", Commute: "#EAB308", "School Expenses": "#888C78", Others: "#E2E2D5" };
const CATS = ["Food", "Commute", "School Expenses", "Others"];
const TT_STYLE = { background: "var(--pd-surface)", border: "1px solid var(--pd-border)", borderRadius: ".75rem", color: "var(--pd-text)", fontSize: ".8rem" };
              ...
              <div className="card-body">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={trendData}>
                    ...
                    <Line type="monotone" dataKey="amount" stroke="var(--pd-primary)" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: "var(--pd-primary)" }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="card-body">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={barData} barCategoryGap="30%">
                    ...
                    <Bar dataKey="budget" name="Budget" fill="rgba(149,193,89,.25)" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="actual" name="Actual" fill="var(--pd-primary)" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
                ...
                <div className="col-4" key={s.label}>
                  <div className="card rounded-3 h-100 glass-card">
                    <div className="card-body text-center p-2">
                      <p className="text-uppercase small fw-bold text-secondary mb-1" style={{ fontSize: ".6rem", whiteSpace: "nowrap", letterSpacing: "-0.2px" }}>{s.label}</p>
                      <p className="fw-bold text-white mb-0" style={{ fontSize: ".9rem", fontWeight: 800 }}>{s.value}</p>
                    </div>
                  </div>
                </div>
              ))}
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={4}>
                        {pieData.map((e) => <Cell key={e.name} fill={CAT_COLORS[e.name] || "var(--pd-primary)"} />)}
                      </Pie>
                      <Tooltip formatter={(v) => formatCurrency(v)} contentStyle={TT_STYLE} />
                      <Legend wrapperStyle={{ fontSize: ".75rem", color: "#94a3b8" }} />
                    </PieChart>
            ...
            <div className="card rounded-3 mb-3" style={{ background: "rgba(149,193,89,.08)", border: "1px solid rgba(149,193,89,.2)" }}>
              <div className="card-body py-3">
                <div className="d-flex align-items-center justify-content-between mb-2 gap-2">
                  <span className="fw-bold text-white" style={{ fontSize: ".8rem", whiteSpace: "nowrap" }}>🌐 Exchange Rates</span>
                  {ratesDate && <span className="text-secondary" style={{ fontSize: ".65rem", whiteSpace: "nowrap" }}>Update: {ratesDate}</span>}
                </div>
                {ratesError ? (
                    ...
                    {CURRENCIES.map((cur) => (
                      <div className="col-12" key={cur.code}>
                        <div className="d-flex align-items-center justify-content-between py-1" style={{ borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                          ...
                          <span className="fw-bold" style={{ color: "var(--pd-primary)", fontSize: ".875rem" }}>₱{Number(rates[cur.code]).toLocaleString()}</span>
                        </div>
                      </div>
                    ))}
                  </div>
